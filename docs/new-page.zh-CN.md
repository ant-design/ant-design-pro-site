...
项目启动后你会在右下角看到一个小气泡，那是 umi UI 的快捷入口，点击后你可以看到 umi UI 的页面。你可以通过点击进入 `资产 -> 模板` 查看模板列表，选择合适的页面添加到项目中。

umi 会自动将对应目标的资源下载并添加到项目中，同时修改路由配置。当前该过程还不可逆，所以执行该操作前还请将已有改动提交到 Git。目标添加成功后就可以按照你的具体需求调整相关代码了。
